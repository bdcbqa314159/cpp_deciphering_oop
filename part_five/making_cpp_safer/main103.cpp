...
// Note use of const member function will not allow any member of 'this' to be modified
// Note that const instances may only call const member functions. Of course, all instances can call const member functions -- in these
// cases, it is a guarantee that we won't change those instances!
...
int main()
{
    ...
    studentBody.push_back(Student("Hana", "Sato", 'U', "Miss", 3.8, "C++", "178PSU"));
    studentBody.push_back(Student("Sam", "Kato", 'B', "Mr.", 3.5, "C++", "272PSU"));
    ...
    for (int i = 0; i < studentBody.size(); i++)
        studentBody[i].Print(); // print vector1’s contents

    cout << "Everyone to earn a PhD" << endl;
    // the auto keyword will allow iter's type to be determined by its first usage (in for loop)
    // Notice how auto (further below) simplifies the iterator which would otherwise be defined by:
    // for (vector<Student>::iterator iter = studentBody.begin(); iter != studentBody.end(); ++iter)
    for (auto iter = studentBody.begin(); iter != studentBody.end(); ++iter)
        (*iter).EarnPhD();
